...
用于在完整口播写作前取得当前口播声音真源和经过确认的完整口播案例，也用于用户明确要求时初始化、接入和维护 visual-multimedia 自己的私人库。独立钩子由另一套脚本、目录、数据类型和索引维护。普通写作只读；保存、更新声音或写入案例必须由用户明确要求。
...
接入已经存在的 v3 私人库时使用 `adopt --root <位置>`。脚本把唯一根目录保存在当前用户的 `.visual-multimedia/voiceover-reference-library-config.json`；配置只是指针，正文资源仍全部位于私人库。

初始化只建立 v3 库标识和真实案例索引，不生成示范声音、占位案例或假装真实的创作材料。随后由独立钩子脚本建立自己的空索引。
...
寻找完整案例时先打开 `<私人库>/口播案例索引.md`，按写作任务和成品语境浏览。范围仍大时用普通文本搜索标题、全文和隐藏标签：

```powershell
rg -n -i "主题|动作|结果" "<私人库>\完整口播案例"
```

打开多份方向不同且真正相关的完整口播，再把实际路径交给完整口播写作。独立钩子通过其专用索引读取，写作上层可以把两类原文共同交给口播写作。参考数量由差异、质量和上下文容量决定；索引不评分、不预先淘汰候选，也不选择唯一模仿对象。没有合适参考时沿当前材料继续，不凑数。

完整案例不提供当前事实、作者身份和立场。声音真源和合格声音候选可以证明表达习惯，但不提供本次事实。声音检索与案例检索必须保持分开。
...
- 配置能够重新定位唯一根目录；
- v3 manifest、声音真源、完整案例与案例索引结构一致；
- `voice_eligible` 只出现在允许的写作来源；
- 完整口播没有同文重复；
- 索引来自真实资源，不含占位正文。

脚本自测只证明私人库协议、写入、资格约束、案例索引和去重，不证明模型已经读过资源，也不证明一份生成口播自然。真实写作验证还必须运行 `show` 和 `voice-candidates` 读取正式私人库，再对一项真实口播请求交付可观察的审查或成稿结果。

完成维护后返回私人库根目录、声音真源状态、案例数、声音候选数和案例索引实际路径。除非用户同时要求写作或制作，库维护完成后停止。
