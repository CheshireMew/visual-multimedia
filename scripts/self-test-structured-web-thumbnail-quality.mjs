...
const testRoot = path.join(skillRoot, "artifacts", "self-tests", "structured-web-thumbnail-quality");
...
if (Number(sourceManifest.quality?.thumbnail?.minimum_text_px) < 14) {
  throw new Error(
    "结构化网页布局模板把主要阅读文字的实际显示下限设在 14px 以下"
  );
}
...
function expectFailure(fixtureRoot, expectedRule, expectedText) {
  ...
  if (result.status === 0
    ...
    throw new Error(
      `结构化网页缩略图质量回归没有按预期失败：${fixtureRoot}\n${output}`
    );
  }
}
...
console.log(
  "结构化网页缩略图质量回归通过：布局模板主要文字不低于 14px；缺少完整缩略图合同会触发 S7，实际显示字过小会触发 Q7 硬失败。"
);
