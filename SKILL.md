...
- 用户已经指定静态卡、封面或信息图时，由 `references/social-card-production.md` 唯一负责内容容量、首张视觉稿、画布、修改和静态验收。
...
已确认最终上图文字时不再改写；只有材料或长稿时先由 `clean-copy` 形成并确认上图文字。静态任务按阅读目的选择文字、表格、图形、图片或组合；图形仅在更易理解或用户明确要求时使用。文案确认后默认直接生成使用真实内容、内容决定尺寸与样式的视觉稿；用户明确要求作图前先看文字方案时，才提交信息顺序、表达形式、宽高、字体、配色和结构并停止。只要求文案时交付后停止。
...
先定表达形式，再采用已确认项目风格、品牌、明确要求的完整参考或用户选定的个人视觉资源。没有时，按内容、受众和载体设计，不套“中性基线”、案例样式或题材预设。内置配方只有被点名或专项路径要求时才采用。普通卡在首张视觉稿落实样式；只有结构化网页和系列复用才建立项目风格档案。
...
普通静态卡只读取 `references/social-card-production.md`，按需读取配色、字体、模板或素材说明；用 `scripts/static-card-workbench.mjs` 建立单 HTML 和两种预览，不读取其它媒体说明。结构化网页与时间型视觉才读取视觉配方、网页制作和语义运动说明。文字动效需要时由 `scripts/text-motion-library.mjs` 选择效果，并物化 `assets/text-motion-library/text-motion-runtime.js` 与 `assets/text-motion-library/text-motion-binding.js`。
