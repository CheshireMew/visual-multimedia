...
所有生成任务先用 `node scripts/media-task-workspace.mjs ensure --task-id <稳定任务-id>` 建立本 Skill 自己的 `artifacts/<task-id>/` 工作区。制作真源、运行项目、导入副本、中间文件、预览、报告、渲染和最终成品都留在这个被 Git 忽略的任务目录；本文及各 reference 中的“项目目录”都指该目录或其子目录。当前终端目录、活动 Git 仓库、输入素材所在项目、从某个项目调用本 Skill，以及用户给出的外部交付位置，都不构成生产目录，也不能改变这条路径。
