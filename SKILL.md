...
所有生成任务先估算本轮新增与同时存在的峰值字节，再用 `node scripts/media-task-workspace.mjs preflight --task-id <稳定任务-id> --expected-bytes <字节>` 通过容量闸门，随后用同一参数执行 `ensure`，建立本 Skill 自己的 `artifacts/<task-id>/` 工作区。制作真源、运行项目、导入副本、中间文件、预览、报告、渲染和最终成品都留在这个被 Git 忽略的任务目录；可跨任务复用的大型派生缓存只能进入本机配置声明且有容量上限的缓存根，任务目录保存其内容身份和运行报告，不复制缓存实体。本文及各 reference 中的“项目目录”都指该目录或其子目录。当前终端目录、活动 Git 仓库、输入素材所在项目、从某个项目调用本 Skill，以及用户给出的外部交付位置，都不构成生产目录，也不能改变这条路径。任务结束后运行 `inventory` 和 `finalize --status completed|failed|interrupted`，把实际占用与清理候选写回任务清单；没有删除授权时只报告候选。

用户明确要求把成品放到外部目录时，仍先在 Skill 内完成制作、真实检查和确认，只把用户点名的最终文件复制到准确交付位置；Skill 内真源、报告和已验证成品继续保留，外部副本不成为后续修改入口。外部产品自己的安装、缓存和内部工程状态继续服从该产品的正式配置，但必须把可移交真源、项目快照或交付结果收回当前任务目录。用户没有点名外部交付时，不向调用仓库或其它项目写入任何生成文件。
...
- 用户已经指定静态卡、封面或信息图时，由 `references/social-card-production.md` 唯一负责内容容量、完整正向视觉参考确认、画布、修改和静态验收；没有完整正向参考时停止，不开始作图。
...
用户要建立自己的二次元口播角色时，先读取 `references/anime-avatar-production.md`。只有母版而没有校准视频时，先把 `assets/anime-avatar-prompts/motion-source-video.md` 的完整提示词和合格条件交给用户；用户要求完整制作且外部生成已获授权时直接代为提交。用户已经提供校准视频时，由 Agent 检查真实画面、指出不合格的具体时间段，或继续完成固定裁切、联系表、视觉口型标注、机器文件和验证。不得要求用户逐帧查看、排列原视频帧、编辑 JSON、运行内部命令或确认技术计划。

这条无骨骼路径使用校准视频的完整人物帧，适合动作克制、普通话口型大体同步的角色。用户同时要求明显人物动作和高精度口型时，在生成素材前说明两者不能由当前完整帧重组稳定兼得，停止这条路径并让用户决定是否改用音频驱动或分层、骨骼角色系统。角色建库本身不套用长视频五阶段；母版身份、长期注册和最终成品样片分别在真正影响用户结果时确认，`confirm-plan` 只确认 Agent 已检查技术计划。
...
已确认最终上图文字时不再改写；只有材料或长稿时先由 `clean-copy` 形成并确认上图文字。静态任务按阅读目的选择文字、表格、图形、图片或组合；图形仅在更易理解或用户明确要求时使用。文案确认后先确认完整正向视觉参考；参考确认通过后，才生成使用真实内容、内容决定尺寸与样式的视觉稿。用户还要求作图前看文字方案时，一并提交信息顺序、表达形式、宽高、字体、配色和结构并停止。只要求文案时交付后停止。
...
先定表达形式。普通静态卡只采用已确认项目或系列的完整成品、品牌提供的完整视觉示例、用户明确指定的完整正向参考，或用户已经选定的个人视觉资源；没有完整正向参考时停止，不由 Agent 自行造风格。配色卡、字体预设、布局模板、局部截图、负面案例和内置配方都不能代替完整参考。结构化网页、时间型视觉和专项路径仍按各自规则选择配方；只有结构化网页和系列复用才建立项目风格档案。
...
普通静态卡把第一张真实完整画布作为唯一设计确认稿；确认后修正反馈并完成最终检查与点名导出，不重复提交空壳方案或第二轮画布。用户明确要求作图前文字确认时先停止。其它高成本媒体仍按对应 reference 制作必要样稿。
...
普通静态卡只读取 `references/social-card-production.md`，在完整正向参考确认通过后按需读取配色、字体、模板或素材说明，并用 `scripts/static-card-workbench.mjs` 建立单 HTML 和两种预览；确认前不创建画布，也不读取其它媒体说明。结构化网页与时间型视觉才读取视觉配方、网页制作和语义运动说明。文字动效需要时由 `scripts/text-motion-library.mjs` 选择效果，并物化 `assets/text-motion-library/text-motion-runtime.js` 与 `assets/text-motion-library/text-motion-binding.js`。
