...
description: 把用户已经确认的主题、内容真源或现有素材转化为可修改、可预览、可导出的社交卡、封面、图表、GIF、代码动画、可手动推进的多场景 HTML、Live Photo、视频、音频或播客，并负责审查、起草和改写相应的标题、口播、字幕、节目结构与媒体配套帖子，以及维护不随 Skill 发布的口播私人库。Use when the user asks to 选择具体媒体载体、先审查或写媒体文案再制作视觉、诊断或改写口播中的 AI 组织痕迹、初始化或维护口播私人库、把卡片或动画做成可编辑网页、制作手动/自动/混合播放的 HTML 演示与动画、制作多比例封面或轮播图、导出图片、GIF 或 Live Photo、制作无实拍视频、按参考视频复刻、对齐或比较成片、从单张图或角色设定建立无需 Live2D 的二次元角色母版、校准素材和语音时间轴、用已注册二次元角色生成完整口播轨或固定角色窗、把访谈原声与背景说明和逐段解释制作成视频、剪辑已有实拍/访谈/录屏素材，或编辑音频与播客；不用于 PPTX、从长材料中筛选什么值得分享、独立纯文字短帖与 Thread、主动研究或核查主题、撰写长文与 Newsletter、规划拍摄、社交媒体运营、上传或发布。
---
...
媒体文案默认由当前模型起草和修改。用户明确点名 DeepSeek 或其它写作生产者时才切换，并服从该入口的可用性与失败边界；无论使用哪个生产者，都消费同一份净化材料、作者声音和创作参考，并经过同一轮成稿核查与听众回读。逐字引用、数据标签、来源名称和用户确认保持原样的文字直接传递。
...
| 任务 | 读取资源 | 读取目的 |
| --- | --- | --- |
| 用户明确把已经确认的长文或长内容制作成视频 | `references/video-direction-contracts.md` | 绑定来源快照、内容简报、确认旁白、发音表和机器可读场景计划，但不复制剪辑时间线 |
...
口播私人库的正式入口是 `scripts/voiceover_reference_library.py`，候选选择使用 `voice-candidates`。二次元口播只通过 `scripts/anime-avatar-project.py`、`scripts/render-anime-avatar.py` 和 `scripts/compose-anime-avatar-inset.py`，注册库位于 `assets/anime-avatar-libraries/`；“夜希数字人”按注册资源采用。采访原声讲解型先 `list-profiles`，再沿 `plan → confirm-plan → render → review → finalize` 的唯一活动链执行。
