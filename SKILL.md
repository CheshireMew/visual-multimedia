...
先选择写作动作。只审查时判断可用性、保留项和问题根因，交付诊断后停止。新写、扩写、实质重组或审查后改写完整口播时，净化现实材料，分别读取可靠声音证据、多份完整案例和独立钩子原文；用最值得听众转述的具体关系开头，以事实、动作或结果兑现，再从内容、结构、作者与听众、听感回读。典型 AI 句式只作为上层问题的症状，不建禁词表。错字、格式、等义压缩、短标题、数据标签、简短字幕和其它确定性派生走轻量路径。确认全文不因局部反馈重写；只修改用户点名的层面，并修复由此直接受影响的衔接。
