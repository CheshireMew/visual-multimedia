...
静态卡、封面和轮播页先把观众最终会看到的标题、正文、标签、来源、署名或水印全部写完整，再提交一份制作确认：准确文字、必要元素、相对位置与阅读顺序、目标比例、字体家族与字重、色彩和表面风格。用户确认后才创建网页；用户已经明确授权直接完成或连续制作时可以继续。用户要求简洁、干净或留白时，调整的是视觉噪音、层级和密度；已经确认的核心主张、主要支撑、必要行动和指定原句继续完整保留。
...
制作前先完成成品需要的标题、口播、字幕、节目结构和配套帖子。内容真源固定引语、数字、归因和事实边界；表达可以按听众理解重组，但画面与声音不能替空洞文案补课。能够脱离媒体独立成立的短帖、Thread 或短文案交给 `100x-learning`；与当前媒体共同完成表达的帖子留在本流程。

先写清各通道的信息职责。首次出现听众可能不认识的人、产品、工具或概念时，先用一句自然的话说明它是什么、与当前内容有什么关系，再展开特征或做法。旁白承担主要解释时，单独听也能知道对象、关键关系和结论；画面再展示空间、状态、数量和情绪。视觉主导的作品可以少说，但要说明哪些理解必须从画面获得。样稿只验证对应层面，不反过来截短确认内容。

先选择写作动作。只审查时判断可用性、保留项和问题根因，交付诊断后停止。新写、扩写、实质重组或审查后改写完整口播时，净化现实材料，分别读取可靠声音证据、多份完整案例和独立钩子原文；用最值得听众转述的具体关系开头，以事实、动作或结果兑现，再从内容、结构、作者与听众、听感回读。典型 AI 句式只作为上层问题的症状，不建禁词表。错字、格式、等义压缩、短标题、数据标签、简短字幕和其它确定性派生走轻量路径。

每轮修改先综合原始请求、活动内容真源、有效确认、硬要求和最新反馈。用户用例子说明完整度、可懂度或实操程度时，默认只说明质量，不缩成例子；明确限定范围才收窄。局部反馈保留有效上游；内容、受众、张数或比例等上游失效时，回到最早受影响层重做，不在错误内容或构图上美化。
...
网页案例保存已经成立的制作证据、结构容量和验收方法。只有用户或当前项目明确采用某个案例的视觉语言时，它才参与字体、颜色、材质和构图；内容题材相似只用于找到可能有帮助的制作证据，不构成视觉采用。当前内容、已确认风格档案和用户反馈始终决定最终画面。

### 6. 用可展示成果分阶段确认

静态成品和短任务在最早未确认层做样稿：文案确认主张，风格样稿确认视觉语言，构图样稿确认信息骨架，动效或声音样稿确认代表性变化。用户已要求静态成品时，构图样稿同时承载待确认的完整文案和目标比例，在完整画布中暴露密度、阅读顺序、留白和裁切，不用局部裁切或多张候选代替。提交后停止；明确授权连续完成才继续网页包和导出。确认后沿同一真源扩展，不把样稿复制成完整成品。批量任务按条目推进，共同上游变化时说明影响范围。
